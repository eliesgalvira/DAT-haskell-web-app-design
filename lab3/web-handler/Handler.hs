...
instance Functor Handler where
    ...
    fmap f h = HandlerC $ \ req st0 -> do
        -- Monad IO:
        (x, st1) <- runHandler h req st0
        pure (f x, st1)
...
dispatchHandler handler req respond = do
    ...
    let scValue = mkSetCookieValue $ hsSession st1
        wairesp = case res of
            HRHtml html ->
                let headers = [ (W.hContentType, mimeHtml)
                              , (W.hSetCookie, scValue) ]
                in W.responseBuilder W.ok200 headers (renderHtmlBuilder html)
            HRRedirect url ->
                let headers = [ (W.hLocation, T.encodeUtf8 url)
                              , (W.hContentType, mimeText)
                              , (W.hSetCookie, scValue) ]
                in W.responseBuilder W.seeOther303 headers (T.encodeUtf8Builder "Redirect")
            HRError status msg ->
                let headers = [ (W.hContentType, mimeText) ]
