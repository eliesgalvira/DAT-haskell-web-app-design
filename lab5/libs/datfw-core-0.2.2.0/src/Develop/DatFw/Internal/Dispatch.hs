...
dispatchHandler handler env croute request respond = do
    ...
    hresp <- runHandler rhenv handler request sessionMap
...
runHandler rhenv handler request session = do
    stateRef <- newIORef (HandlerState session Nothing 1 id)
    ...
    let finalSession = hsSession state
        headers = hsHeaders state []
    case result of
        Right (TypedContent t c) ->             -- Normal response
            pure (Normal (rheDefStatus rhenv) headers t c finalSession)
        Left (HCError err) -> do                -- Error
            Normal status headers' t c finalSession' <- rheOnError rhenv err request finalSession
            pure (Normal status (headers ++ headers') t c finalSession')
        Left (HCRedirect status url) -> do      -- Redirect
            let disable_caching hs =
                    ("Cache-Control", "no-cache, must-revalidate")
                    : ("Expires", "Thu, 01 Jan 1970 05:05:05 GMT") : hs
                headers' = (if status == H.movedPermanently301 then id
                            else disable_caching) $
                              ("Location", T.encodeUtf8 url) : headers
