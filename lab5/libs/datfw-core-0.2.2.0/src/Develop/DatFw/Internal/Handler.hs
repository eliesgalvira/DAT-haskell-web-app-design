...
modifySession :: MonadHandler m => (SessionMap -> SessionMap) -> m ()
modifySession f = modify $ \st -> st{ hsSession = f (hsSession st) }

addHeaderInternal :: MonadHandler m => Header -> m ()
addHeaderInternal h = modify $ \st -> st{ hsHeaders = hsHeaders st . (h :) }
