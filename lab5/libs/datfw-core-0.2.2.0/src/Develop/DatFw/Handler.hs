...
lookupGetParams name = do
    ...
    pure $ T.decodeUtf8 <$> catMaybes (lookup' name' query)
...
lookupHeaders :: MonadHandler m => Text -> m [B.ByteString]
lookupHeaders name = do
    headers <- requestHeaders <$> getRequest
    let name' = CI.mk $ encodeUtf8 name
    pure $ lookup' name' headers

lookupHeader :: MonadHandler m => Text -> m (Maybe B.ByteString)
lookupHeader name =
    listToMaybe <$> lookupHeaders name


lookup' :: Eq a => a -> [(a, b)] -> [b]
lookup' k = map snd . filter ((k ==) . fst)
...
addHeader :: MonadHandler m => Text -> B.ByteString -> m ()
addHeader n v = addHeaderInternal (CI.mk $ encodeUtf8 n, v)
